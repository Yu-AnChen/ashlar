...
def main(argv=sys.argv):
    ...
    parser.add_argument(
        '--scales', metavar='SCALE', nargs='*',
        help=('a sequence of numbers indicating different scales of the images'
              'the length of the sequence must be the same as the number of'
              'cycles')
    )
    parser.add_argument(
    ...
    if len(scales) != len(filepaths):
        print_error(
            "Wrong number of scales. Must be {} but get {}"
            .format(len(filepaths), len(scales))
        )
        return 1
...
def process_single(
    ...
    num_channels = [False for i in scale_set]
    ...
    for idx, scale in enumerate(scale_set):
        ...
        first = (num_channels[idx] is False)
        mosaic_args_final['first'] = True if first else False
